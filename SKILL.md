...
- `fonts/JetBrainsMono*.woff2` — JetBrains Mono variable font (code + technical/tabular + structural primary labels + compact actions)
...
1. **Type** — Inter is the default for interface prose, supporting copy, and display. JetBrains Mono is the code + technical/tabular family, and it also carries two explicit bounded roles: the **structural primary label** (24px / 300 / 1.12 line-height / -0.02em tracking, implemented canonically by `.surface-title` and `.surface-panel-title`) and the **compact action** (14px / 300 / 1.20 line-height / 0 tracking, implemented canonically by `.surface-action`). The compact action is Caption-*sized*, not the Caption role — no uppercase, no wide tracking, no 400 weight, and never Inter. Body prose and supporting copy stay Inter. Do not apply mono generically merely because something is interface-facing, and do not derive a third mono role from these two. Weights by role: generic H1/H2 display 400; H3, structural primary labels and compact actions 300; body 200 ExtraLight at 24px; supporting copy Small Inter 300. Defined role sizes may differ — never invent an ad-hoc size merely for emphasis. A structural primary label stays 300 even when its semantic element is an h1 or h2. The style guide's 18px Inter **CTA** specimen is a separate role and is not conformed to the compact action.

...
1. **Type** — Inter is the default for interface prose, supporting copy, and display. JetBrains Mono is the code + technical/tabular family, and it also carries two explicit bounded roles: the **structural locator** (24px / 300 / 1.12 line-height / -0.02em tracking, implemented canonically by `.surface-title`) and the **compact action** (14px / 300 / 1.20 line-height / 0 tracking, implemented canonically by `.surface-action`). The **panel primary label** (`.surface-panel-title`) carries the identical 24px / 300 / 1.12 / -0.02em metric but is **Inter**. Family is owned by the selector, never by the label's text: `.surface-title` is mono in both of the shell's title forms — the root-level plain heading and the breadcrumbed subpage title — and `.surface-panel-title` is Inter whatever the panel is called, a technical or structural name included. The compact action is Caption-*sized*, not the Caption role — no uppercase, no wide tracking, no 400 weight, and never Inter. Body prose and supporting copy stay Inter. Do not apply mono generically merely because something is interface-facing, and do not derive a third mono role from these two. Weights by role: generic H1/H2 display 400; H3, both primary-label implementations and compact actions 300; body 200 ExtraLight at 24px; supporting copy Small Inter 300. Defined role sizes may differ — never invent an ad-hoc size merely for emphasis. A primary label stays 300 even when its semantic element is an h1 or h2. The style guide's 18px Inter **CTA** specimen is a separate role and is not conformed to the compact action.

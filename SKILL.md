...
1. **Type** — Inter is the default for interface prose, supporting copy, and display. JetBrains Mono is the code + technical/tabular family, and it also carries the explicit **structural primary-label role**: 24px / 300 / 1.12 line-height / -0.02em tracking, implemented canonically by `.surface-title` and `.surface-panel-title`. Body prose and supporting copy stay Inter. Do not apply mono generically merely because something is interface-facing. Weights by role: generic H1/H2 display 400; H3 and structural primary labels 300; body 200 ExtraLight at 24px; supporting copy Small Inter 300. Defined role sizes may differ — never invent an ad-hoc size merely for emphasis. A structural primary label stays 300 even when its semantic element is an h1 or h2.

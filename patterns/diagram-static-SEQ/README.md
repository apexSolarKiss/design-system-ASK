...
This is a **Class A static** diagram scaffold (system / architecture diagram templates), distinct from the **Class B** project-output artifact pattern. It is the **third sibling** alongside `patterns/diagram-static-H/` (horizontal left→right cascade) and `patterns/diagram-static-V/` (vertical centered spine). Same data grammar, same visual contract, same PNG export, same inheritance discipline — only the placement geometry differs.

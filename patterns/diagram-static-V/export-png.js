...
(function () {
  ...
  /* Preference 1: the DS-owned embedded-font carrier (window.DSA_EMBEDDED_FONTS,
     ...
     is the path that works under file://. Used only when it covers every @font-face
     the page declares (when those are readable — http); under file:// the page CSS
     is cross-origin-unreadable so pageFaces is empty and the DS-owned carrier is
     trusted wholesale. Returns null when the carrier is absent, malformed, or has
     drifted behind the page's declared faces (→ fall through to the fetch path). */
  function carrierCss(pageFaces) {
    ...
    for (var i = 0; i < c.length; i++) {
      ...
      have[f.family + '|' + weight + '|' + style] = true;
      css += faceRule(f.family, style, weight, f.b64);
    }
    for (var j = 0; j < pageFaces.length; j++) {
      var pf = pageFaces[j];
      if (!have[pf.family + '|' + pf.weight + '|' + pf.style]) return null; // carrier short of the page
    }

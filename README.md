...
| `patterns/_diagram-shared/` | **Canonical shared support for the static diagram engines** — currently one member, `diagrams-text-layout.js`, the text-layout contract (measurement, role metrics, caps, deterministic breaking, wrapped height, tspan emission) consumed by `diagram-static-H` / `-V` / `-SEQ`. A **plane, not a mandate**: the folder name confers no authority, each member declares its own target set, and `diagram-static-FLOW` and `diagram-interactive-spine` are explicitly excluded. Members are mirrored into the consuming bundles by `tools/sync-diagram-shared.mjs`; the mirrors are generated artifacts, never hand-edited, and `--check` exits non-zero on divergence |

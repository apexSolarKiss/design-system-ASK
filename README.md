...
- **Shadows** are long and soft. Three steps: `sm` (1px), `md` (24px), `lg` (60px). No hard drop shadows, no inner shadows, no colored shadows. Zero-blur interaction rings — the focus glow and the attention edge's 0.5px ring — are edge paint, not elevation shadows, and this line does not govern them.
...
### Hover, press, focus
- **Hover** — opacity drops 1.0 → 0.92, and/or the border changes. **No arbitrary new hue is introduced**: where a border changes color, it resolves either to something the element already carries or, for the two shaped populations below, to the palette's emphasis magenta. The treatment is **role-driven**, and the roles genuinely differ — do not conform one to another:

...
**The two roles differ on leading, and only on leading.** Both `surface-shell` title forms — plain and breadcrumb — take **1.16**, a metric the shell owns for the wrapping, underlined structural locator; the panel label keeps `--lh-heading` (1.12), being a single line with no such condition. The breadcrumb's older pattern-local `1.35` is **retired**: it was measured against an underline that was a border and survived the change to a text decoration by inertia. The retired scope was `.surface-breadcrumb .surface-title` — the pattern's **root-level plain** title and `.surface-panel-title` both keep 1.12.
...
- **Hover** — opacity drops 1.0 → 0.92, and/or the border changes. **No arbitrary new hue is introduced**: where a border changes color, it resolves to something the element already carries. The treatment is **role-driven**, and the roles genuinely differ — do not conform one to another:
  ...
  | --- | --- |
  | generic anchors (foundation `a`), **compact actions** (`.surface-action`) and **full-panel links** (`a.surface-panel`) | **foreground-bound** — the operative border resolves to `currentColor` |
  ...
  | **shell footer destinations** | none of their own — they are compact actions and take the row above |

  **One mechanism, three anatomies, and not one paint.** Each of those three keeps its own border shape and its own governed foreground: a generic anchor resolves a text-edge border to its inherited color, a compact action resolves its complete pill border to `--fg-1` (or `--fg-2` on the `--secondary` variant), and a full-panel link resolves its complete panel border to `--fg-1`. A panel's hover border is therefore stronger than a secondary button's — one mechanism, two role priorities — and demoting the panel to match a button pixel-for-pixel would be matching the wrong thing.

  The reason is scale. A `--line-2` rest border is the faintest line the system has, and brightening it to `--line-1` moves between two strengths of one translucent value: a real change that reads as almost none on the light gradient, especially under a simultaneous opacity drop. Resolving to the object's own foreground is what makes the response legible.

  **The list is closed.** These three populations share the mechanism; a future bordered control does not inherit it by being interactive, and joins only by being named here.

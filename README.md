...
- **Focus** — never the browser default. Across the live-surface modules and the shell there are **four anatomies by role**, which are not interchangeable; artifact patterns such as `message-archive` own pattern-local indicators:
  ...
  | **disclosure triggers** (`details.surface-disclosure > summary`) | a `--fg-1` outline around the trigger, 2px wide and `--space-1` out, with the summary at `--fg-1` and its indicator turned `--ask-emphasis-magenta`; the outline shows in every case: open or closed, and when the trigger leads with a mark; no edge, no glow |
...
| `patterns/output-artifact/` | conforming | the Class B document scaffold on the document register. Its title, lede, section titles, body and list text, labels, metadata, code, quotation, preformatted block, dense table and disclosure take the register's roles and treatments, loaded as the four modules in the sealed-use order below, and the rendered check passes on its generated preview. Its own CSS sets no type metric and no text foreground, and writes its geometry at zero specificity. It draws only the geometry the register leaves to a consumer — the column, the metadata strip's and provenance footer's layout, list and table geometry, and rules — in the Class B line-intensity token `--artifact-line`, stronger than the foundation hairline in light. That token never reaches a text foreground, a passage rail or the disclosure's panel; it lies outside the type roles, compositions and treatments a disposition measures, and it is no variant for another surface. `tools/check-type-roles.mjs` governs its template's typography declarations |

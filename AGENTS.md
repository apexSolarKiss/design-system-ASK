...
This repo's `AGENTS.md` is a **resolved carrier**: the shared execution protocol (below, resolved from `control-surface/protocol/AGENTS.shared.md`) + the `core-ecology` profile + the design-system-ASK-local delta. The shared block is machine-maintained — do not hand-edit between the markers.
...
<!-- BEGIN local-delta -->
## Design-System-ASK-Local

These rules are the repo-local delta for `design-system-ASK` — owner of the design foundations, the specialized opt-in semantic primitives, the artifact-inheritance scaffolds, and the surface patterns — on top of the shared protocol + `core-ecology` profile above. `design-system-ASK`'s local delta wins for `design-system-ASK` where an explicit conflict exists.

Two consumption lifecycles run through the rules below, and they are not interchangeable. An **artifact-scaffold contract** is inherited at generation time: a downstream consumer generates an output, seals it, and freezes it for audit. A **surface-pattern contract** frames a live deployed surface: the consumer takes it by same-repo reference or as a pinned local vendored copy, seals nothing, and re-syncs when the owner contract changes. `README.md` §Scope defines the material types; this delta carries the workflow consequences. A surface pattern is not a scaffold, and it is not a further artifact class.
...
- the relevant `patterns/*/README.md` for any pattern change — artifact scaffold or surface pattern alike
...
- **Propagation caution.** This repo is upstream of multiple public, operator-side, and private/firewalled consumers. A change to tokens, engines, `diagrams.css`, export scripts, Spectral State, Evidence State, Three Functions, fonts, logo assets, an artifact-scaffold contract, or a surface-pattern contract may require downstream re-sync. Either contract kind can create that obligation. Enumerate the affected consumer class in the change summary and flag the propagation obligation.
- **Foundation discipline.** Tokens, typography, palette, role names, logo logic, artifact-scaffold contracts, and surface-pattern contracts are identity-bearing. Do not add, rename, remove, or reinterpret them without explicit ASK source-of-intent.
- **Consumer ownership boundary.** `design-system-ASK` owns the shared source files, engines, CSS, export scripts, and canonical pattern contracts in both lifecycles. What the consumer owns differs:
  - **Artifact-scaffold consumers** own their source data, artifact generation, sealing, committed rasters/PDFs/HTMLs, render stamps, and frozen artifacts.
  - **Surface-pattern consumers** own their Tier 3, their payload, their deployment, and their local instance and payload adaptations. They consume by same-repo reference or by a pinned local vendored copy, re-sync when the owner contract changes, and seal nothing — the surface stays live.
...
The owner-delta manifest names: the landed SHA; the changed files and their blobs; the affected material type or pattern group — a pattern carries an artifact class only when it is an artifact scaffold; the expected live-render impact; the expected export/raster impact; the known neutral classes or material; and the consumer classes requiring census.

Record the manifest and propagation state **operator-side**, in `ecology-ASK-EXTERNAL/design-system-ASK_consumer-ledger.md` (already the propagation ledger per §Required reads), honoring the wall and public-registry discipline: no pins, private/firewalled consumer names, future-state claims, or operator-only propagation details enter the public `CONSUMERS.md`, which stays landed-public-only. This extends the existing **Propagation caution** bullet under §Repo-specific rules (which already establishes the enumerate-affected-consumers-and-flag obligation) with the standing mechanism; it does not duplicate or supersede it. Consumer adaptation remains under each consumer's own governing contract: generation, sealing, stamps, and rasters for an artifact-scaffold consumer; re-sync and local adaptation of the live surface for a surface-pattern consumer.
<!-- END local-delta -->

...
- Routed handoffs into this surface land in the **shared intake** `ecology-ASK-EXTERNAL/sources of intent/` with `-TBI`; recipient-side ingestion renames off `-TBI` in place. When classification requires a durable closure, that closure is recorded in the **shared ecology scratch** `ecology-ASK-EXTERNAL/scratch/`. A co-located `-SUPERSEDED` intake artifact is retired lineage — never queue membership and never an ingestion event.
...
These rules are the repo-local delta for `design-system-ASK` (the diagram/foundation owner) on top of the shared protocol + `core-ecology` profile above. `design-system-ASK`'s local delta wins for `design-system-ASK` where an explicit conflict exists.
